...
import { Button, Card, Avatar, Spinner } from "@heroui/react";
import Link from "next/link"; // Replaced @heroui/react link with native Next.js link
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (isPending) {
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-slate-50 dark:bg-slate-950">
        <Spinner size="md" color="primary" label="Assembling Workspace..." />
      </div>
    );
  }
      ...
      <div className="flex h-screen w-screen items-center justify-center bg-slate-50 dark:bg-slate-950 p-4">
        <Card className="max-w-md w-full border border-slate-200/60 dark:border-slate-800 shadow-sm bg-white dark:bg-slate-900">
          <Card.Content className="p-6 text-center space-y-4">
            ...
            <Button as={Link} href="/login" color="primary" className="w-full font-medium">Authentication Portal</Button>
          </Card.Content>
        </Card>
      </div>
    );
  ...
  // 3. Dynamic Route Matching Engine based on the URL pathname track
  ...
  if (pathname.startsWith("/dashboard/admin")) {
    ...
    // If they are adjusting profile settings, determine links by their auth role claim instead
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-slate-50 dark:bg-slate-950 font-sans antialiased text-slate-800 dark:text-slate-200">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex flex-col border-r border-slate-200/60 dark:border-slate-800/80 bg-white dark:bg-slate-900 transition-all duration-300 ease-in-out ${isSidebarOpen ? "w-60" : "w-16"}`}>
        ...
        <div className="flex h-14 items-center justify-between px-4 border-b border-slate-100 dark:border-slate-800/50">
          ...
          <Button isIconOnly variant="light" size="sm" onClick={() => setIsSidebarOpen(!isSidebarOpen)} className={!isSidebarOpen ? "hidden" : "text-slate-400 hover:text-slate-600"}>
        ...
        <nav className="flex-1 space-y-0.5 p-2.5 overflow-y-auto">
          {!isSidebarOpen && (
            <Button isIconOnly variant="light" className="w-full h-10 mb-2 text-slate-400" onClick={() => setIsSidebarOpen(true)}>
              <Menu className="h-4 w-4" />
            </Button>
          )}
          ...
          {activeLinks.map((link) => {
            ...
            return (
              <Button
                ...
                className={`w-full justify-start gap-3.5 text-xs font-medium h-10 px-3 rounded-lg ${
                  isActive 
          ...
          <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800/40">
            <Button
              ...
              variant={pathname === "/dashboard/my-profile" ? "flat" : "light"}
              color={pathname === "/dashboard/my-profile" ? "primary" : "default"}
              className={`w-full justify-start gap-3.5 text-xs font-medium h-10 px-3 rounded-lg ${
