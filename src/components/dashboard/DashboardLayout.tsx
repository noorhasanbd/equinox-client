...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const ownerLinks: SidebarItem[] = [
    ...
    { label: "Financial Reports", href: "/dashboard/earnings", icon: BarChart3 },
  ...
  if (isPending) {
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-neutral-50 dark:bg-neutral-950">
        <Spinner label="Verifying session..." color="primary" />
      </div>
    );
  }
      ...
      <div className="flex h-screen w-screen items-center justify-center bg-neutral-50 dark:bg-neutral-950 p-4">
        <Card className="max-w-md p-6 text-center border border-neutral-200 dark:border-neutral-800">
          <Card.Content>
            ...
            <p className="text-sm text-neutral-500 mb-4">You must be logged in to view your dashboard.</p>
            <Button as={Link} href="/login" color="primary">Go to Login</Button>
          </Card.Content>
        </Card>
      </div>
    );
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100">
          ...
          {activeLinks.map((link) => {
            ...
            return (
              <Button
                ...
                href={link.href}
                variant="light"
                className={`w-full justify-start gap-4 text-sm h-11 px-3 ${
          ...
          <div className={`flex items-center gap-3 p-2 rounded-xl ${!isSidebarOpen ? "justify-center" : ""}`}>
            ...
            {isSidebarOpen && (
              <div className="flex flex-col min-w-0 overflow-hidden">
                ...
                <span className="text-[9px] bg-blue-50 dark:bg-blue-950/50 text-blue-600 dark:text-blue-400 font-medium px-1.5 py-0.5 rounded w-max mt-1 uppercase tracking-wider">{userRole}</span>
              </div>
            )}
          </div>
      ...
      <div className={`flex flex-col flex-1 h-full overflow-hidden transition-all duration-300 ${isSidebarOpen ? "pl-64" : "pl-16"}`}>
